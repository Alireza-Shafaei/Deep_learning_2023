...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        """
        x, w, b = cache
        dx = torch.matmul(dout, w.T).view(x.size())
...
class Linear_ReLU(object):
    ...
    def backward(dout, cache):
        ...
        d_relu = ReLU.backward(dout, relu_cache)

        dx, dw, db = Linear.backward(d_relu, linear_cache)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh1, grads['W2'], grads['b2'] = Linear.backward(d_scores, cache_scores)
        _, grads['W1'], grads['b1'] = Linear_ReLU.backward(dh1, cache_h1)

        for param_name in ['W1', 'W2']:
          loss = loss + self.reg * torch.sum(self.params[param_name] ** 2)
          grads[param_name] = grads[param_name] + 2 * self.reg * self.params[param_name]
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(1, self.num_layers + 1):
            ...
            #print(i,self.params[f'W{i}'].shape)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers + 1):
            ...
            scores, cache = Linear.forward(scores, Wi, bi)
            layer_caches.append(cache)

            if i < self.num_layers:
                scores, relu_cache = ReLU.forward(scores)
                layer_caches.append(relu_cache)
                
                if self.use_dropout:
                  dropout_mask = (torch.rand_like(scores) >= self.dropout_param['p']).float()
                  scores = scores * dropout_mask / (1 - self.dropout_param['p'])
        ...
        for i in reversed(range(1, self.num_layers + 1)):
            if self.use_dropout:
                dropout_mask = layer_caches.pop()
                d_scores = d_scores * dropout_mask / (1 - self.dropout_param['p'])

            if i < self.num_layers:
                d_scores = ReLU.backward(d_scores, layer_caches.pop())

            Wi, bi = self.params[f'W{i}'], self.params[f'b{i}']
            d_scores, dWi, dbi = Linear.backward(d_scores, layer_caches.pop())
            grads[f'W{i}'] = dWi + self.reg * Wi 
            grads[f'b{i}'] = dbi
            #print(grads[f'W{i}'].shape, grads[f'b{i}'].shape)
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
